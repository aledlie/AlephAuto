...
export class AstGrepPatternDetector {
  ...
  async detectPatterns(repoPath, detectConfig = {}) {
    ...
    try {
      ...
      // Normalize matches
      const normalized = matches.map(m => this.normalizeMatch(m));
      ...
      proc.on('close', (code) => {
        if (code === 0 || code === null) {
          try {
            ...
            const matches = lines.map(line => JSON.parse(line));

            resolve(matches);
          } catch (error) {
            logger.warn({ stderr }, 'Failed to parse ast-grep output, returning empty results');
            resolve([]);
          }
        } else {
  ...
  normalizeMatch(match) {
    return {
      ...
      file_path: match.file || match.path,
      line_start: match.range?.start?.line || match.line,
      ...
      matched_text: match.text || match.matched,
